// Live remote OpenCode acceptance (OAuth against the hosted MCP). SKIPS unless
...
test("opencode remote OAuth login completes headlessly and preserves the four-tool identity", async (t) => {
  if (!process.env.EGA_REMOTE_ACCEPTANCE) {
    t.skip("set EGA_REMOTE_ACCEPTANCE=1 to run the live remote OpenCode acceptance");
    return;
  }
  assert.ok(process.env.EGA_OPENCODE_CONFIG_HOME, "EGA_OPENCODE_CONFIG_HOME is required");
  assert.ok(process.env.EGA_OPENCODE_DATA_HOME, "EGA_OPENCODE_DATA_HOME is required");
  ...
  assert.equal(summary.client_id_kind, "opaque-uuid", "OpenCode must register via DCR");
