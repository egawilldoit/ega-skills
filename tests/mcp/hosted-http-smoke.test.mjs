...
test("hosted CLI serves a real built release over HTTP", async (t) => {
  ...
  const child = spawn(process.execPath, ["packages/mcp/bin/ega-mcp-hosted.mjs"], {
    ...
    env: { ...process.env, PORT: String(port), EGA_HOSTED_ARTIFACT_DIR: build.registryHome, EGA_HOSTED_BEARER_TOKEN: "smoke-token", EGA_HOSTED_ALLOW_STATIC_TOKEN: "true", EGA_HOSTED_AUTHZ_FILE: authzPath, EGA_HOSTED_ALLOWED_ORIGINS: "http://localhost" },
