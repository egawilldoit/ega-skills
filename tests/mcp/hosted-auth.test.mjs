...
test("JWKS verifier validates issuer, audience, scope and signature with bounded cache", async () => {
  ...
  const principal = await verify(token(privateKey, "k1", claims), new AbortController().signal);
  assert.deepEqual(principal, { subject: "user-1", scopes: ["ega:read"] });
  await verify(token(privateKey, "k1", claims), new AbortController().signal);
  assert.equal(fetches, 1);
  await assert.rejects(() => verify(token(privateKey, "k1", { ...claims, aud: "wrong" }), new AbortController().signal));
});
